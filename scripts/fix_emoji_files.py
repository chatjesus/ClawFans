"""Fix emoji encoding in page.tsx and Sidebar.tsx"""
import sys
...
open(r'C:\Users\PRO\Desktop\CUDA\synclub-local\frontend\src\app\page.tsx', 'w', encoding='utf-8').write(page_content)
...
def replace_unicode_escapes(s):
    def repl(m):
        ...
        # Handle surrogate pairs: replace with actual emoji via surrogateescape
        if 0xD800 <= code <= 0xDBFF:
            # High surrogate - look ahead for low surrogate
            return m.group(0)  # keep as-is, handle below
        return chr(code)
    # Handle surrogate pairs together
    def repl_pairs(m):
        high = int(m.group(1), 16)
        low = int(m.group(2), 16)
        code_point = 0x10000 + (high - 0xD800) * 0x400 + (low - 0xDC00)
        return chr(code_point)
    s = re.sub(r'\\u([dD][89aAbB][0-9A-Fa-f]{2})\\u([dD][cCdDeEfF][0-9A-Fa-f]{2})', repl_pairs, s)
    s = re.sub(r'\\u([0-9A-Fa-f]{4})', lambda m: chr(int(m.group(1), 16)), s)
    return s

sidebar_content = replace_unicode_escapes(sidebar_content)
open(r'C:\Users\PRO\Desktop\CUDA\synclub-local\frontend\src\components\Sidebar.tsx', 'w', encoding='utf-8').write(sidebar_content)
